...
// spaceBytes is a read-only run of spaces used by spaces() to emit indentation
// without allocating (the previous fmt-based "%*s" formatting boxed its args).
var spaceBytes = []byte("                                                                ")

// spaces writes n spaces to the buffer.
func (p *printer) spaces(n int) {
	for n >= len(spaceBytes) {
		p.Write(spaceBytes)
		n -= len(spaceBytes)
	}
	if n > 0 {
		p.Write(spaceBytes[:n])
	}
}
